...
def _resource_typing_xmap(params,
  ...
  def gather_resources(axes):
    result = set()
    for axis in axes: result |= set(inner_axis_resources[axis])
    return result
  for v, axes in zip(call_jaxpr.outvars, params['out_axes']):
    broadcast_axes = set(axes) - set(v.aval.named_shape)
    used_resources = gather_resources(v.aval.named_shape)
    for baxis in broadcast_axes:
      baxis_resources = gather_resources(baxis)
